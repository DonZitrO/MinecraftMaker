	...
	private BaseEntity transformNbtData(BaseEntity state) {
		...
		if (tag != null) {
			...
			if (hasTilePosition) {
				...
				CompoundTagBuilder builder = tag.createBuilder().putInt("TileX", newTilePosition.getBlockX())
						.putInt("TileY", newTilePosition.getBlockY()).putInt("TileZ", newTilePosition.getBlockZ());
				...
				if (hasDirection || hasLegacyDirection || hasFacing) {
					...
					if (direction != null) {
						Vector vector = transform.apply(direction.toVector()).subtract(transform.apply(Vector.ZERO))
								.normalize();
